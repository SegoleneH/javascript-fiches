...
// Elle peut être dans l'un des trois états suivants : 
// en attente (pending),résolue (fulfilled) ou rejetée (rejected).

// Exemple :
const myPromise = new Promise((resolve, reject) => {
// Effectuez des opérations asynchrones
    const data = fetchData();
    if (data) {
        resolve(data); // Promesse résolue avec les données
    } else {
        reject("Error fetching data."); // Promesse rejetée avec un message d'erreur
    }
});
...
const myPromise1 = new Promise((resolve, reject) => {
    ...
    if (randomNumber > 0.5) {
        resolve(randomNumber); // Promesse résolue avec le nombre aléatoire
    } else {
        reject("Number is too small."); // Promesse rejetée avec un message d'erreur
    }
    });
myPromise
    .then((number) => {
